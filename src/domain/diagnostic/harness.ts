 ...
 *   2. one guarded block per test, each printing a single marker line saying what happened.
 *
...
/** The name the guarded block binds the exception to. Unlikely to collide with a learner's. */
const CAUGHT = '_diagnostic_failure'

/** Indices only, so no escaping is needed. */
function passLine(index: number): string {
  return `print(${JSON.stringify(`${MARKER}:pass:${String(index)}>>>`)})`
}
 ...
 */
function failLine(index: number): string {
  const prefix = JSON.stringify(`${MARKER}:fail:${String(index)}:`)
  return `print(${prefix} + type(${CAUGHT}).__name__ + ">>>")`
}
...
export function buildTestProgram(learnerCode: string, tests: readonly HarnessTest[]): string {
  ...
  return [learnerCode, '', ...blocks, ''].join('\n')
}
...
export function parseTestReport(stdout: string, tests: readonly HarnessTest[]): TestReport {
  ...
  for (const line of stdout.split('\n')) {
    ...
    seen.set(Number(match[2]), { passed: match[1] === 'pass', error: match[3] ?? null })
  }
